...
local function write_json(path, value)
  ...
  if not handle then return false, "cannot write update journal" end
...
local function read_json(path)
  ...
  if not handle then return nil, "cannot read update journal" end
  local value = handle.readAll()
  handle.close()
  local ok, decoded = pcall(textutils.unserializeJSON, value)
  if not ok or type(decoded) ~= "table" then return nil, "update journal is invalid" end
  return decoded
end
...
function M.recover(journal_path)
  ...
  if journal.phase == "activating" or journal.phase == "awaiting-confirmation" then
    ...
    if fs.exists(journal_path) then fs.delete(journal_path) end
    if journal.backupPath and fs.exists(journal.backupPath) then fs.delete(journal.backupPath) end
...
function M.confirm(journal_path, journal)
  if fs.exists(journal_path) then fs.delete(journal_path) end
  if journal and journal.backupPath and fs.exists(journal.backupPath) then fs.delete(journal.backupPath) end
  return true
end
